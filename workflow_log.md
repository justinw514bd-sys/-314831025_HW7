...
### 3. 錯誤排除與 Debug
* **開發者 Prompt** (直接貼上終端機的錯誤追蹤碼)：
> openai.AuthenticationError: Error code: 401 - {'error': {'message': 'Missing Authentication header'}}
> google.api_core.exceptions.NotFound: 404 models/gemini-1.5-flash is not found
...
* **問題**：在申請 API 金鑰時，誤入 Google Cloud 的「服務帳戶」與「專案權限限制」的複雜介面，導致金鑰被鎖死。
* **解決方案**：Agent 指引開發者轉由專屬的「Google AI Studio」快速通道重新申請 `AQ.` 開頭的萬能金鑰，免除所有不必要的權限設定綁定。

**3. 解決 500 JSON 解析崩潰與 404 模型退役問題**
* **問題**：伺服器頻繁拋出 `500 Internal Server Error` 以及 `404 Not Found` (因 1.5 版模型退役)。
* **解決方案**：Agent 協助利用原生 SDK 的 `generation_config` 強制鎖定 JSON 輸出，並將環境變數更新為最新世代的 `gemini-2.5-flash`，最終打通任督二脈，實現系統穩定運行。
